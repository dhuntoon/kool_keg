...
static void gpio_task_example(void* arg)
{
    ...
    for(;;) {
        if(xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
        	...
        	gpio_intr_disable(17); // Disable GPIO interrupts in case to prevent button bouncing as well as rapid presses
...
//                eeprom_write_byte(eeprom_address, starting_address, firsttemp);
//                printf("Wrote byte 0x%02X to address 0x%04X\n", single_write_byte1, starting_address);
//                vTaskDelay(20/portTICK_PERIOD_MS);
//                // EEPROM single byte write second half of temp
//                uint8_t single_write_byte2 = secondtemp;
//                eeprom_write_byte(eeprom_address, starting_address+1, secondtemp);
//                printf("Wrote byte 0x%02X to address 0x%04X\n", single_write_byte2, starting_address+1);
//                vTaskDelay(20/portTICK_PERIOD_MS);
//
//
//                // WRITE VOLUME
...
//                eeprom_write_byte(eeprom_address, starting_address+2, firstvol);
//                printf("Wrote byte 0x%02X to address 0x%04X\n", single_write_byte3, starting_address+2);
//                vTaskDelay(20/portTICK_PERIOD_MS);
//                // EEPROM single byte write second half of vol
//                uint8_t single_write_byte4 = secondvol;
//                eeprom_write_byte(eeprom_address, starting_address+3, secondvol);
//                printf("Wrote byte 0x%02X to address 0x%04X\n", single_write_byte4, starting_address+3);
//                vTaskDelay(20/portTICK_PERIOD_MS);
//
//
//                // READ
//                // EEPROM single byte read first half of temp
//                uint8_t random_read_byte1 = eeprom_read_byte(eeprom_address, starting_address);
//                printf("Read byte 0x%02X at address 0x%04X\n", random_read_byte1, starting_address);
//                vTaskDelay(20/portTICK_PERIOD_MS);
//                // EEPROM single byte read second half of temp
//                uint8_t random_read_byte2 = eeprom_read_byte(eeprom_address, starting_address+1);
//                printf("Read byte 0x%02X at address 0x%04X\n", random_read_byte2, starting_address+1);
//                vTaskDelay(20/portTICK_PERIOD_MS);
//                // EEPROM single byte read first half of vol
//                uint8_t random_read_byte3 = eeprom_read_byte(eeprom_address, starting_address+2);
//                printf("Read byte 0x%02X at address 0x%04X\n", random_read_byte3, starting_address+2);
//                vTaskDelay(20/portTICK_PERIOD_MS);
//                // EEPROM single byte read second half of vol
//                uint8_t random_read_byte4 = eeprom_read_byte(eeprom_address, starting_address+3);
//                printf("Read byte 0x%02X at address 0x%04X\n", random_read_byte4, starting_address+3);
//                vTaskDelay(20/portTICK_PERIOD_MS);
//
...
void app_main(void)
{
	i2c_master_init();
	vTaskDelay(1000 / portTICK_PERIOD_MS);
    MLX90614_SMBusInit(18, 19, 100000); // sda scl and 50kHz
    lcd_init();
